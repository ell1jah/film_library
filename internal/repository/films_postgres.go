...
import (
	...
	"github.com/ell1jah/film_library/internal/utils"
...
func (f *FilmsPostgres) InsertFilm(film *models.Film) (int, error) {
	insertQuery := `INSERT INTO films (title, description, release_date, rating) VALUES ($1, $2, $3, $4) RETURNING film_id;`
	releaseDate, err := utils.StringToDate(film.ReleaseDate)
	if err != nil {
		return 0, err
	}
...
func (f *FilmsPostgres) GetFilms(attrs *map[string]any) (*[]models.Film, error) {
	...
	if title, exists := (*attrs)["title"]; exists {
		...
		if !ok {
			return &[]models.Film{}, fmt.Errorf("incorrect type of title fragment")
		}
	...
	if _, exists := (*attrs)["actor"]; exists {
		if actorName, ok := (*attrs)["actor"].(string); ok {
			...
			return &[]models.Film{}, fmt.Errorf("incorrect type of actor name fragment")
		}
	}
	...
	if order, exists := (*attrs)["order"]; exists {
		...
		if !ok {
			return &[]models.Film{}, fmt.Errorf("incorrect order format")
		}
		lenOrder = len(orderSlice)
		for _, param := range orderSlice {
			valid := regexp.MustCompile("^[A-Za-z0-9_]+$")
			if !valid.MatchString(param) {
				return &[]models.Film{}, fmt.Errorf("invalid order attribute, might be sql injection")
			}
	...
	if asc, exists := (*attrs)["ascending"]; exists {
		boolAsc, ok := asc.(bool)
		if !ok {
			return &[]models.Film{}, fmt.Errorf("incorrect type of ascending attribute")
		}
		if boolAsc {
...
func (f *FilmsPostgres) UpdateFilm(film *map[string]any) error {
	...
	for k, v := range *film {
		...
		if k == "actors" {
			...
			actors := make([]models.Actor, 0, len(actorsAny))
			for _, actor := range v.([]any) {
				...
				actorID, err := utils.СonvertFloat64ToInt(actorIDFloat64)
				if err != nil {
					return err
				}
				newActor := models.Actor{ActorID: actorID}
				actors = append(actors, newActor)
			}
...
func (f *FilmsPostgres) InsertFilmActors(filmID int, actors []models.Actor) error {
