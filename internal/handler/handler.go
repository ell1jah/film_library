...
import (
	...
	_ "github.com/ell1jah/film_library/docs"
...
func (h *Handler) InitRoutes() error {
	...
	http.Handle("/actors/add", h.Auth(http.HandlerFunc(h.addActor)))
	http.Handle("/actors/update/", h.Auth(http.HandlerFunc(h.updateActor)))
	http.Handle("/actors/delete/", h.Auth(http.HandlerFunc(h.deleteActor)))

	http.Handle("/films/get", http.HandlerFunc(h.getFilms))
	http.Handle("/films/add", h.Auth(http.HandlerFunc(h.addFilm)))
	http.Handle("/films/update/", h.Auth(http.HandlerFunc(h.updateFilm)))
	http.Handle("/films/delete/", h.Auth(http.HandlerFunc(h.deleteFilm)))
...
func (h *Handler) DecodeActor(r *http.Request) (*models.Actor, error) {
...
func (h *Handler) DecodeFilm(r *http.Request) (*models.Film, error) {
...
func (h *Handler) DecodeAdmin(r *http.Request) (*models.Admin, error) {
